        ...
        for j in range(tablero["tamanio"]):
            ...
            if j == i:
                ...
                if (pos in malas_nivel_facil) or (pos_invertida in malas_nivel_facil):
                    ...
                    casillas_especiales[(i, j)] = {
                        "color": ("white", "black"),
                        "texto": "F " + str(mala_actual),
                        "modificador": mala_actual,
                    }
                    ...
                    window[pos].Update(
                        "P x3" if pos in multiplicador_nivel_facil else "P x2", button_color=("white", "purple")
                    )
                    casillas_especiales[(i, j)] = {
                        "color": ("white", "purple"),
                        "texto": "P x3" if pos in multiplicador_nivel_facil else "P x2",
                        "modificador": 23 if pos in multiplicador_nivel_facil else 22,
                    }
            elif nivel == "medio":
                if (pos in malas_nivel_medio) or (pos_invertida in malas_nivel_medio):
                    ...
                    window[pos].Update(
                        "P x3" if pos_invertida in multiplicador_nivel_medio else "P x2",
                        button_color=("white", "purple"),
                    )
                    casillas_especiales[(i, j)] = {
                        "color": ("white", "purple"),
                        "texto": "P x3" if pos in multiplicador_nivel_facil else "P x2",
                        "modificador": 23 if pos_invertida in multiplicador_nivel_medio else 22,
                    }
            elif nivel == "difícil":
                if (pos in malas_nivel_dificil) or (pos_invertida in malas_nivel_dificil):
                    ...
                    window[pos].Update(
                        "P x3" if pos in multiplicador_nivel_dificil else "P x2", button_color=("white", "purple")
                    )
                    casillas_especiales[(i, j)] = {
                        "color": ("white", "purple"),
                        "texto": "P x3" if pos in multiplicador_nivel_facil else "P x2",
                        "modificador": 23 if pos in multiplicador_nivel_dificil else 22,
                    }
